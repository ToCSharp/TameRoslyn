...
{
    public class TameBaseParameterListSyntax : TameBaseRoslynNode
    {
        public static string TypeName = "BaseParameterListSyntax";
        private SeparatedSyntaxList<ParameterSyntax> _parameters;
        private int _parametersCount;
        private bool _parametersIsChanged;
        private string _parametersStr;
        private List<TameParameterSyntax> _taParameters;

        public TameBaseParameterListSyntax(string code)
        {
            Node = SyntaxFactoryStr.ParseBaseParameterList(code);
            AddChildren();
        }

        public TameBaseParameterListSyntax(BaseParameterListSyntax node)
        {
            Node = node;
            AddChildren();
        }

        public TameBaseParameterListSyntax()
        {
        }
        ...
        public SeparatedSyntaxList<ParameterSyntax> Parameters
        {
            get
            {
                ...
                {
                    _parameters =
                        SyntaxFactory.SeparatedList(TaParameters?.Select(v => v.Node).Cast<ParameterSyntax>());
        ...
        public List<TameParameterSyntax> TaParameters
        {
            get
            {
                if (_taParameters == null)
                {
                    _taParameters = new List<TameParameterSyntax>();
                    foreach (var item in _parameters)
                        _taParameters.Add(new TameParameterSyntax(item) {TaParent = this});
                }
        ...
        {
            Kind = Node.Kind();
            _parameters = ((BaseParameterListSyntax) Node).Parameters;
